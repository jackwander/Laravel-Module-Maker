...
class MakePolicy extends Command
{
    ...
    protected $signature = 'jw:make-policy {name} {--module=}';
    protected $description = 'Create a new Policy file for a specific module';
    ...
    {
        $basePath = config('module-maker.paths.modules', app_path('Modules'));
        $modulePath = "{$basePath}/{$moduleName}/Policies";
        $baseNamespace = config('module-maker.namespaces.root', 'App\\Modules');
        $namespace = "{$baseNamespace}\\{$moduleName}\\Policies";

        if (!$this->files->exists($modulePath)) {
            $this->files->makeDirectory($modulePath, 0755, true);
        }

        $fileName = Str::endsWith($name, 'Policy') ? $name : $name . 'Policy';
        $filePath = "{$modulePath}/{$fileName}.php";

        if (!$this->files->exists($filePath)) {
            $content = $this->getStubContent('policy', [
                'namespace' => $namespace,
                'class' => $fileName,
            ]);

            $this->files->put($filePath, $content);
            $this->info("Policy file {$filePath} created successfully.");
        } else {
            $this->info("Policy file {$filePath} already exists.");
        }
    }
}
