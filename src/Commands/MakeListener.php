...
class MakeListener extends Command
{
    ...
    protected $signature = 'jw:make-listener {name} {--module=}';
    protected $description = 'Create a new Listener file for a specific module';
    ...
    {
        $basePath = config('module-maker.paths.modules', app_path('Modules'));
        $modulePath = "{$basePath}/{$moduleName}/Listeners";
        $baseNamespace = config('module-maker.namespaces.root', 'App\\Modules');
        $namespace = "{$baseNamespace}\\{$moduleName}\\Listeners";

        if (!$this->files->exists($modulePath)) {
            $this->files->makeDirectory($modulePath, 0755, true);
        }

        $fileName = Str::endsWith($name, 'Listener') ? $name : $name . 'Listener';
        $filePath = "{$modulePath}/{$fileName}.php";

        if (!$this->files->exists($filePath)) {
            $content = $this->getStubContent('listener', [
                'namespace' => $namespace,
                'class' => $fileName,
            ]);

            $this->files->put($filePath, $content);
            $this->info("Listener file {$filePath} created successfully.");
        } else {
            $this->info("Listener file {$filePath} already exists.");
        }
    }
}
